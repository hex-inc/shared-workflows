...
2. Socket Policy Sync
    ...
    - Runs hourly, on pushes to `main` that change `socket/**`, and via `workflow_dispatch`.
    - Fetches live policies from the Socket API, compares for effective equivalency, and overwrites Socket when they drift (repo wins). UI edits in Socket are reverted within about an hour (or immediately on the next push/dispatch).
    ...
    - See [`socket/README.md`](socket/README.md) for license deny-by-default semantics.
